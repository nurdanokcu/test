...
const educationalContent = ref<TypeBlog[]>([]);
const localePath = useLocalePath();
onMounted(() => {
  isLoading.value = false;
  educationalContent.value = educational.data.slice(0, 3);
});
</script>
...
<template>
  <div class="articles">
    <h2 class="articles-title">
      {{ $t("admin.resources.videos") }}
    </h2>
    <div class="posts-lists">
      <div v-if="isLoading" class="drafts-loading">
        <AdminLoadingBoxes :amount="3" height="72px" />
      </div>
      <TransitionGroup
        v-if="educationalContent.length > 0"
        tag="ul"
        name="fade"
        class="articles-list"
      >
        <AdminResourcesListItem
          v-for="item in educationalContent"
          :key="String(item.id)"
          :item="item"
    ...
    <CommonButtonGradientFilled
      :text="$t('admin.resources.allYoutube')"
      :padding="'16px'"
      :border-radius="'0px 0px 16px 16px'"
      :font-options="{ textTransform: 'uppercase' }"
      :is-link="true"
      :path="localePath('/admin/resources/videos')"
    >
      <template #icon>
